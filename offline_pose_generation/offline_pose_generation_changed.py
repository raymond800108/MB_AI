...
parser.add_argument("--num_mesh", type=int, default=1000, help="Number of frames to record similar to MESH dataset")
parser.add_argument("--num_dome", type=int, default=1000, help="Number of frames to record similar to DOME dataset")
parser.add_argument(
    "--dome_interval",
    type=int,
    default=1,
...
parser.add_argument("--output_folder", "-o", type=str, default="output_640x480_3", help="Output directory.")
...
class RandomScenario(torch.utils.data.IterableDataset):
    def __init__(
        ...
        self.head_parts = []
        self.train_parts = []
        self.train_parts_need_randomazation=[]
        self.train_part_mesh_path_to_prim_path_map = {}
        self.vehicle_part_mesh_path_to_prim_path_map = {}
        self.head_part_mesh_path_to_prim_path_map = {}
    ...
    def _setup_camera(self):
        ...
        # self.writer_helper.register_camera_data(self.camera, config_data=config_data)
    ...
    def _setup_vehicle_object(self):
        vehicle_part_idx = 0
        # Add the part to train the network on
        vehicle_name = "MB_297V"
        ref_path ="/home/raymondyao/.local/share/ov/pkg/isaac-sim-2023.1.1/MB297V.usd"
        prim_type="MB_297V"
        path = "/World/" + prim_type

        mesh_path = path + "/" + prim_type
        name = f"vehicle_part_{vehicle_part_idx}"

        self.vehicle_part_mesh_path_to_prim_path_map[mesh_path] = path

        vehicle_part = DynamicObject(
            usd_path=ref_path,
            prim_path=path,
            mesh_path=mesh_path,
            name=name,
            position=np.array([0.0, 8.0, 90.0]),
            scale=np.array([1.0, 1.0, 1.0]),
            mass=1.0,
        )
        
        vehicle_part.prim.GetAttribute("physics:rigidBodyEnabled").Set(False)

    def _setup_train_objects(self):
        # Add the part to train the network on
        part_name = "xreal_with_usd_mat"
        ref_path ="/home/raymondyao/.local/share/ov/pkg/isaac-sim-2023.1.1/xreal_with_usd_mat.usd"

        # train part
        prim_type="xreal_with_usd_mat"
        path = "/World/" + prim_type
        mesh_path = path + "/" + prim_type
        name = f"train_part_{0}"
        ...
        # #part of train obj need randomazation
        # r_prim_type="anna"
        # r_path = "/World/" + r_prim_type
        # r_mesh_path = r_path + "/"+r_prim_type
        # r_name = f"train_part_{1}"

        # self.train_part_mesh_path_to_prim_path_map[r_mesh_path] = r_path

        # train_part_need_randomazation = DynamicObject(
        #     usd_path=ref_path,
        #     prim_path=r_path,
        #     mesh_path=r_mesh_path,
        #     name=r_name,
        #     position=np.array([0.0, 0.0, 0.0]),
        #     scale=config_data["OBJECT_SCALE"],
        #     mass=1.0,
        ...
        # train_part_need_randomazation.prim.GetAttribute("physics:rigidBodyEnabled").Set(False)
        # self.train_parts_need_randomazation.append(train_part_need_randomazation)

        # # Add semantic information
        # r_mesh_prim = world.stage.GetPrimAtPath(r_mesh_path)
        # add_update_semantics(r_mesh_prim, r_prim_type)
    ...
    def randomize_material(self,prim):
        prims = rep.get.prims(path_pattern=prim)
        mats = rep.create.material_omnipbr(
            metallic=rep.distribution.uniform(0.0, 1.0),
            roughness=rep.distribution.uniform(0.0, 1.0),
            diffuse=rep.distribution.uniform((0, 0, 0), (1, 1, 1)),
